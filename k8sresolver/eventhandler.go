...
func (h *EventHandler) OnAdd(obj interface{}) {
	...
	if !ok {
		longresolver.GetDfLogger().Errorf("%v is not an object with type *v1.Endpoints", obj)
		return
	}
...
func (h *EventHandler) OnUpdate(oldObj, newObj interface{}) {
	...
	if !ok {
		longresolver.GetDfLogger().Errorf("%v is not an object with type *v1.Endpoints", oldObj)
		return
	}

	newEndpoints, ok := newObj.(*v1.Endpoints)
	if !ok {
		longresolver.GetDfLogger().Errorf("%v is not an object with type *v1.Endpoints", newObj)
		return
	}
...
func (h *EventHandler) Update(endpoints *v1.Endpoints) {
	...
	old := h.endpoints
	h.endpoints = make(map[string]bool)
	for _, sub := range endpoints.Subsets {
		for _, point := range sub.Addresses {
			h.endpoints[point.IP] = true
		}
...
func diff(o, n map[string]bool) bool {
	if len(o) != len(n) {
		return true
	}
